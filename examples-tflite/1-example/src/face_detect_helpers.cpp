...
void init_regions()
{
  ...
  int x = (int)(display_width * 0.05);
  int y = (int)((display_height - height) / 2);
  left_region = Rect(x, y, width, height);

  width = (int)(display_width * 0.3);
  height = width;
  x = (int)(display_width * 0.1 + frame.cols);
  y = (int)((display_height - height) / 2) - 200;
  right_region = Rect(x, y, width, height);

  label_region = Rect(x, height + y + 10, width, 50);
  similarity_region = Rect(x, height + y + 70, width, 50);
}

void init_add_button()
{
  add_button = Rect(right_region.x, display_height - 130, 200, 35);
...
void init_stop_button()
{
  ...
  stop_button = Rect(right_region.x, display_height - 75, 100, 35);
...
{
  int x = right_region.x + offset * 40;
  int y = right_region.y + right_region.height + 150 + 40 * row_id;
  Scalar bg_color = Scalar(0, 255, 255);
  for (int i = 0; i < characters.size(); i++)
  {
    input_buttons.push_back(Rect(x + i * 40, y, 30, 30));
